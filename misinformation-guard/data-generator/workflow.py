...
def diversify(category: str):
    ...
    if not isinstance(category, str):
        raise TypeError(f"Expected string for category, got {type(category).__name__}")
...
def extract_output_and_reasoning(result: str) -> tuple:
    ...
    if not isinstance(result, str):
        raise TypeError(f"Expected string for result, got {type(result).__name__}")
...
def sdg(
    ...
    if not isinstance(use_hpu, bool):
        raise TypeError("use_hpu must be a boolean")
    ...
    if not isinstance(prompt_version, str) or prompt_version not in ["v1", "v2"]:
        raise ValueError("prompt_version must be either 'v1' or 'v2'")

    if not isinstance(labels, list) or not all(isinstance(l, str) for l in labels):
        raise TypeError("labels must be a list of strings")

    if not isinstance(category_type, dict):
        raise TypeError("category_type must be a dictionary")
